...
- Redis
- PostgreSQL
- Docker, Nginx Proxy Manager, OCI
...
backend는 recompile시 핫리로드됩니다. (바인드마운트+springBootDevtools+bootRun)  
frontend는 파일 저장시 핫리로드됩니다. (바인드마운트+vite+npm run dev)

## 배포 방법

`.env.example`을 참고하여 `.env.production`을 작성한 후 다음 명령을 실행합니다.
